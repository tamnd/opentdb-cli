// Package cli assembles the opentdb command tree from the opentdb
...
// NewApp assembles the kit application from the opentdb domain. The
...
//
// To add a command, declare it in opentdb/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := opentdb.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(opentdb.Domain{}).Register(app)
