...
describes what users get. Pick a format with `-o`, or let opentdb choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
opentdb <command> -o table   # aligned columns for reading
opentdb <command> -o jsonl   # one JSON object per line, for piping
opentdb <command> -o json    # a single JSON array
opentdb <command> -o csv     # spreadsheet friendly
opentdb <command> -o tsv     # tab-separated
opentdb <command> -o url     # just the URL column
opentdb <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
opentdb <command> --fields id,title,url
```
...
```bash
opentdb <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
opentdb <command>            # a table, because this is a terminal
opentdb <command> | wc -l    # JSONL, because this is a pipe
```
